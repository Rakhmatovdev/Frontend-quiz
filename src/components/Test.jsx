...
if(questionIndex==questions.length){
  toast.success("Congratulation",{icon:"🎉",})
  return <Result title={title} color={color} icon={icon} correctAnswerCount={correctAnswerCount} questionLength={questions.length}/>
}
...
if(answerStatus=="correct" && option ==selectedAnswer){
  ...
  if(option==questions[questionIndex].answer){
    className="correct"
              ...
              return (
                <li key={option}>
                  <label className={`test-label ${className}`}>
                    ...
                    <input type="radio" name="option" onChange={()=>setSelectedAnswer(option)} disabled={statusDisabeled}/>
          ...
          {showNextButton && <button className="btn test-btn" onClick={handleNextQuestion}>{questions.length==answeredQuestions?"Finish":"Next Question"} </button>}
        </form>
      </div>
